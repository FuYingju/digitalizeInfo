  ...
  export default {
    data(){
      return{
        ...
        newProList:[],
        newProFuelList:[], //燃油车数据
        newProElectList:[], //电动车数据
   ...
   methods:{
     ...
     initNewPro(){
       ...
       getHeziNewPro(this.requestParams).then(res => {
         ...
         if(this.newProList != null){
           this.newProFuelList = this.newProList.filter(item => item.tag == '1')
           this.newProElectList = this.newProList.filter(item => item.tag == '0')
    ...
    tagChange(e){
     ...
     ifendcase(row, column, cellValue){
       if(cellValue == 'cover'){
         return '细分市场覆盖率'
       }else{
         return cellValue
